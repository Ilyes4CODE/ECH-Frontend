...
async function refreshToken() {
    try {
        ...
        const response = await fetch(`https://backend.echsahra.com/accounts/token/refresh/`, {
            method: 'POST',
            headers: {
...
async function loadProjectDetails() {
    try {
        ...
        const response = await fetch(`https://backend.echsahra.com/gestion/projects/${projectId}/`, {
            method: 'GET',
            headers: {
...
function generateProjectPdf() {
    ...
    link.href = `https://backend.echsahra.com/gestion/projects/${projectId}/pdf/`; // Adjust URL to match your URL pattern
...
function downloadFile(url, filename) {
    ...
    if (url.includes(`${window.location.hostname}`)) {
        correctedUrl = url.replace(`${window.location.hostname}`, `${window.location.hostname}`);
    } else if (url.startsWith('/upload/')) {
        // If it's a relative URL, make it absolute with correct port
        correctedUrl = `https://backend.echsahra.com${url}`;
    } else if (!url.startsWith('http')) {
        // If it's just a filename or relative path
        correctedUrl = `https://backend.echsahra.com/upload/${url}`;
    }
...
async function updateProject() {
    try {
        ...
        const response = await fetch(`https://backend.echsahra.com/gestion/projects/${projectId}/update/`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
...
async function loadCollaboratorOperations() {
    try {
        ...
        const url = `https://backend.echsahra.com/gestion/caisse/history/?project_id=${projectId}&by_collaborator=true`;
...
async function exportHistoryPDF() {
    ...
    try {
        ...
        const response = await fetch(`https://backend.echsahra.com/gestion/projects/project-finance-pdf/`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
...
async function loadProjectHistory() {
    try {
        ...
        // Use the correct caisse history endpoint
        const url = `https://backend.echsahra.com/gestion/caisse/history/?project_id=${projectId}&by_collaborator=false`;
        console.log('Making request to:', url);
